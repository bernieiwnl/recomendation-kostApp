...
import com.example.rekomendasikostapp.ADMIN.AdminFasilitasKamar;
...
public class UpdateMasterDataKamar extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_master_data_kamar);
        ...
        firebaseFirestore.collection("fasilitas_kamar").document(idFas).addSnapshotListener(UpdateMasterDataKamar.this, new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot documentSnapshot, @Nullable FirebaseFirestoreException e) {
                if(documentSnapshot.exists()){
                    FasilitasKamar fasilitasKamar = documentSnapshot.toObject(FasilitasKamar.class);
                    editNamaFasilitas.setText(fasilitasKamar.getNama_fasilitas());
                    Picasso.get().load(fasilitasKamar.getFoto_url()).into(imageIcon);
                }
                else
                {
    ...
    private void insertFasilitas(){
        ...
        firebaseFirestore.collection("fasilitas_kamar").document(idFas).update(master).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                uploadIcon(idFas);
                Map<String, Object> id = new HashMap<>();
                id.put("idfasilitas" ,idFas);
                firebaseFirestore.collection("fasilitas_kamar").document(idFas).update(id);
                finish();
            }
        });
            ...
            storageTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot , Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if(task.isSuccessful()){
                        ...
                        firebaseFirestore.collection("fasilitas_kamar").document(idFasilitas).update(map);
                    }
                }
            });
        }
