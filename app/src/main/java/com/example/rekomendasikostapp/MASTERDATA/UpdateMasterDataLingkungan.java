...
import com.example.rekomendasikostapp.ADMIN.AdminAksesLingkungan;
...
public class UpdateMasterDataLingkungan extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_master_data_lingkungan);
        ...
        firebaseFirestore.collection("akses_lingkungan").document(idFas).addSnapshotListener(UpdateMasterDataLingkungan.this, new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot documentSnapshot, @Nullable FirebaseFirestoreException e) {
                if(documentSnapshot.exists()){
                    AksesLingkungan aksesLingkungan = documentSnapshot.toObject(AksesLingkungan.class);
                    editNamaFasilitas.setText(aksesLingkungan.getNama_akses());
                    Picasso.get().load(aksesLingkungan.getFoto_url()).into(imageIcon);
                }
                else {
    ...
    private void insertFasilitas(){
        ...
        master.put("nama_akses" , nama_fasilitas);
        firebaseFirestore.collection("akses_lingkungan").document(idFas).update(master).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                uploadIcon(idFas);
                Map<String, Object> id = new HashMap<>();
                id.put("idfasilitas" ,idFas);
                firebaseFirestore.collection("akses_lingkungan").document(idFas).update(id);
                finish();
            }
        });
            ...
            storageTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot , Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if(task.isSuccessful()){
                        ...
                        firebaseFirestore.collection("akses_lingkungan").document(idFasilitas).update(map);
                        Toast.makeText(UpdateMasterDataLingkungan.this,
